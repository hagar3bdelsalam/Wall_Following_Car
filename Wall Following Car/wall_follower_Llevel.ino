...
float Ki               = 0.0;
float Kd               = 3.5;//5.0;
int   TURN_TICKS_RIGHT = 27;
int   TURN_TICKS_LEFT  = 72;
float LEFT_TRIM        = 0.9;
...
void loop(void) {
    ...
    if (distFront > 0 && distFront >= RESET_DEAD_DISTANCE) {
    ...
    if (distFront > 0 && distFront < STOP_DISTANCE) {
        ...
        if (rightOpen) {
            // Turn right: stop first, then pivot.
            deadReverseTrialUsed = false;
            stopMotors(); myDelayMs(100);
            turnRight90();
            return;
        } else if (leftOpen) {
            // Turn left: stop first, then pivot.
            deadReverseTrialUsed = false;
            stopMotors(); myDelayMs(100);
            turnLeft90();
            return;
        } else if (!inDeadRange) {
            // In stop range but NOT in dead range and no open side:
            // Do NOT stop motors — resume PID so the car keeps adjusting.
            if (DEBUG) uart_println(">>> Stop+blocked, not dead range; resume PID.");
            // fall through to PID section below
        } else {
            // Both sides closed AND in dead range.
            stopMotors(); myDelayMs(100);
